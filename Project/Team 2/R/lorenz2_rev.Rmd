...
그래프를 그린다. 
```{r}
(I.W.1 <- ggplot(income.melt, aes(x=cumulated.people, y=cumulated.income, colour=Nation, group=Nation)) + geom_line())
```

이제 piketty의 완전평등선과 소득불평등이 적은 나라의 곡선을 그린다. 
```{r}
(I.W.2 <- ggplot(piketty.melt,aes(x=cumulated.people, y=cumulated.income, colour=difference, group=difference)) + geom_line())
(I.W.2 + geom_line(data=income.melt, aes(x=cumulated.people, y=cumulated.income, colour=Nation, group=Nation)))

```

이제 2가지의 자료를 겹친다. 
```{r}
##(I.W.3 <- ggplot(newdata10.2,aes(x=cumulated.people, y=cumulated.income),)+geom_line()+geom_line(aes(x=cumulated.people,y=cumulated.income),colour=c("difference","Nation")))
##(I.W.3 <- I.W.1 + ggplot(piketty.melt,aes(x=cumulated.people, y=cumulated.income, colour=difference, shape=difference)) + geom_line())
```

하위 정리를 끝내면 상위를 정리한다.
```{r}
##income.piketty<-data.frame(cumulated.people=newdata10[,1] low=piketty[,3] cumulated.income=newdata10[,-1])
```
